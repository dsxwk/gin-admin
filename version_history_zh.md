...
## v3.3.2
> - 优化修复模型软删除
> - 软删除查询兼容底层通过识别是否实现了软删除决定查询条件
> - 新增配置命令行是否生成软删除字段控制是否生成软删除字段
...
## v3.3.1
> - 命令行表格输出样式调整
> - 数据库填充数据命令以及权限同步命令行新增进度条加载
> - 翻译、配置、任务调度优化以及部分功能方法重命名
...
## v3.1.4
> - 合并公共导入和事件相关包
> - 拆分事件总线、业务事件注册表、调试收集器以及Trace存储职责
> - 新增事件和es测试用例
> - 新增上下文获取traceId公共方法
> - 调整修复gRPC默认服务重复注册、i18n语言加载目录为绝对路径
> - 修复以及优化超时中间件
> - 注释修复,测试用例调整
...
> - 优化查询构建器、工具包函数
> - 优化公共响应以及相关涉及后调整
> - 修复grpc模型导致swagger自动生成文档命令失败问题
> - 更新readme文档以及数据库备份文件

## v3.0.1
> - grpc新增方法权限控制,更新所有包版本

## v3.0.0

> - MCP工具优化以及代码格式化、数据库文件备份
> - 新增grpc服务相关功能以及命令行快捷代码生成
...
> - 新增AI Agent模型配置支持openai、deepseek等
> - 新增mcp工具以及ai智能助手
> - 新增命令行创建mcp工具以及mcp工具列表查询
> - 数据表迁移文件以及数据库备份
> - 命令行生成模型文件字段注释转义处理
...
> - 优化队列调试器等
> - 新增job任务支持连接类型redis、kafak、rabbitmq、sync,sync为同步
> - 新增命令行job创建、job任务列表数据获取、job未执行的队列任务清除
