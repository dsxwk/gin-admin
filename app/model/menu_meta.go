...
type MenuMeta struct {
	...
	TransKey    string         `gorm:"column:trans_key;type:varchar(100);not null;default:'';comment:翻译键" json:"transKey"`
	Path        string         `gorm:"column:path;type:varchar(50);not null;comment:路由路径" json:"path"`
	Redirect    string         `gorm:"column:redirect;type:varchar(50);not null;comment:重定向" json:"redirect"`
	...
	IsLink      string         `gorm:"column:is_link;type:json;comment:外链/内嵌时链接地址" json:"isLink"`
	IsIframe    int64          `gorm:"column:is_iframe;type:json;comment:是否内嵌 1=是 2=否" json:"isIframe"`
	Roles       []*RoleMenus   `gorm:"foreignKey:menu_id;references:menu_id;comment:菜单角色" json:"roles"`
